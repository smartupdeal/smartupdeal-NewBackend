        ...
        [
          {
            $lookup: {
              ...
              pipeline: [
                {
                  $match: {
                    $and: [{ $expr: { $eq: ['$$product', '$product'] } }, { user: new Mongoose.Types.ObjectId(userDoc.id) }]
                  }
                }
              ],
              as: 'isLiked'
...
// fetch store products by advancedFilters api
router.post('/listOld', async (req, res) => {
  try {
    let { sortOrder, rating, max, min, category, pageNumber: page = 1 } = req.body;
...
//=========================================== add product api ==============================================//
router.post('/add', auth, role.checkRole(role.ROLES.Admin, role.ROLES.Merchant), upload.single('image'), async (req, res) => 
{
  console.log('productApi', req.body);
  try {
...
router.put('/:id/active', auth, role.checkRole(role.ROLES.Admin, role.ROLES.Merchant), async (req, res) => {
  try {
    const productId = req.params.id;
    const update = req.body.product;
    const query = { _id: productId };
...
// fetch product api
router.get('/getRating/:slug', async (req, res) => {
  try {
     const slug = req.params.slug;
    let productDoc = null;

    productDoc = await Product.aggregate([
      { $match: {slug} },
